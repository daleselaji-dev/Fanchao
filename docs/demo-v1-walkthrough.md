# 《返潮》Demo v1 审阅指南 —— H00-R0《婚宴后场：返席》（2D 行为灰盒）

日期：2026-08-24（自 PR #3 吸收）
状态：**行为合同的审阅与回归文档，仍然有效**；但项目入口与下一步计划以 [docs/game-design/](game-design/README.md) 与 [docs/agent-playbook/](agent-playbook/README.md) 为准。本文提到的「官方主线 UE 5.8」已被双栈裁决取代（见 [game-design/06 §1](game-design/06_技术栈与制作工作流.md#1-技术栈裁决双栈)）。
...
> 定位：Web 切片只验证**行为合同**（空间、任务、航点、让行、复位），不代表美术。技术栈现行裁决：Demo 执行栈 Three.js / 正式产品栈 UE 5.8，见 [game-design/06 §1](game-design/06_技术栈与制作工作流.md#1-技术栈裁决双栈)。
...
| UE 5.8 编译与状态机 | PASS（**历史证据**，2026-08-04 原开发机） | 见交接包 zip 内 `02_当前Demo_H00-R0/原始评审/H00-R0_Round4_行为验证_v0.1.md`；本云端环境无 UE，**未复跑** |
...
1. **Demo 执行栈**：按 [docs/agent-playbook/](agent-playbook/README.md) 在 `demo/web3d/` 制作 Three.js 3D 灰盒 Demo（云端 Agent 可执行）。
2. **正式产品栈（本地开发机）**：在有 UE 5.8 的机器上复验 `Entry` 地图可见画面，出 3 张非黑屏 720p 截图；此任务不阻塞 Demo。
3. 其后的时代底片、异常材质、正式资产按 [game-design/00 阶段规划](game-design/00_项目总览与规划.md#2-阶段规划) 与各 Gate 推进。
